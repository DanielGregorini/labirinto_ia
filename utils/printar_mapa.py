def Printar_mapa(mapa):
    ...
    def mapear_terreno(type_ground, is_agent, is_point, is_secondary):
        ...
        if type_ground == "parede":
            ...
            return f"{cor_solido_plano}██{reset_cor}"
        elif type_ground == "rochoso":
            return f"{cor_rochoso}██{reset_cor}"
        elif type_ground == "pantano":
            return f"{cor_pantano}██{reset_cor}"
        elif type_ground == "arenoso":
            return f"{cor_arenoso}██{reset_cor}"
        
        return " "  # Espaço para terreno desconhecido
    
    for linha in mapa:
        for elemento in linha:
            print(mapear_terreno(elemento.type_ground, elemento.is_agent, elemento.is_point, elemento.is_secondary), end='')
